...
function getSeed(number) {
  ...
  // s can be 1 to 1e6, float
  let s = number % (1e6 - 1) + 1;
  number = floor(number/(1e6 - 1));
  
  // s can be 0.1 to 10, float
...
function mouseClicked() {
  saveCanvas("pw"+String(pw)+"s"+String(s)+"n"+String(n)+"cs"+String(cs)+"quads"+String(quads)+".png");
}
...
//   let colorscheme = random([basic, grass, homebrew, manpage, novel, ocean, pro, redsands, silveraerogel]);
  ...
  // setting from hash:
  [pw,s,n,cs,quads] = getSeed(unhex(hash));
  colorscheme = colorschemes[cs];
  
  // sets some to black and white
  if(abs(n - cs) <= 0.3) {
